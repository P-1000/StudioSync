        ...
        <div className="w-full flex flex-col items-center bg-zinc-50 p-3">
            <div className="flex justify-start border-b w-full p-2 border-zinc-300 px-5">
                {menuitems.map(item => (
                    <button
                        key={item.id}
                        className={`rounded-lg px-4 flex items-center hover:scale-105 transition-all hover:bg-[#5577ffba] hover:text-white gap-2 py-2 mx-2 ${selectedItem === item.id ? 'bg-[#5577FF] text-white shadow border-none' : 'bg-white border'}`}
